...
vec2 matcap(vec3 eyeVector, vec3 surfaceNormal) {
  vec3 reflected = reflect(eyeVector, surfaceNormal);
  float m = 2.8284271247461903 * sqrt(reflected.z + 1.0);
  return reflected.xy / m + 0.5;
}

float lambert(vec3 normalVector, vec3 lightVector) {
  return max(dot(normalize(normalVector), normalize(lightVector)), 0.0);
}
...
void main() {
  ...
  normalVector = vec3(normalVector.x, sqrt(max(0.0, 1.0 - dot(normalVector.xz, normalVector.xz))), normalVector.z);

  vec2 matcapUv = matcap(eye, normalVector).xy;
  float light = lambert(normalVector, lightDirection);
...
export function createPrimaryOrb(viewport, { textureUrl = "/landing-primary/images/ob_texture-old.webp" } = {}) {
  ...
  const targetType = extensions.includes("EXT_color_buffer_float")
    ? FloatType
  ...
  if (!targetType) {
    renderer.dispose();
    throw new Error("PrimaryOrb requires floating-point render targets.");
  }
  ...
  const normalizedMousePosition = new Vector2(-1, -1);
  ...
  const interactionTarget = new WebGLRenderTarget(256, 256, renderTargetOptions);
  const simulationTarget = new WebGLRenderTarget(128, 128, renderTargetOptions);

  const interactionMaterial = new RawShaderMaterial({
    vertexShader: FULLSCREEN_VERTEX_SHADER,
    fragmentShader: INTERACTION_FRAGMENT_SHADER,
    uniforms: {
      time: { value: 0 },
      texture: { value: simulationTarget.texture },
      center: { value: new Vector2(-1, -1) },
      center2: { value: new Vector2(-1, -1) },
      radius: { value: 0.05 },
      strength: { value: 0.05 },
  ...
  const simulationMaterial = new RawShaderMaterial({
    vertexShader: FULLSCREEN_VERTEX_SHADER,
    fragmentShader: SIMULATION_FRAGMENT_SHADER,
    uniforms: {
      texture: { value: interactionTarget.texture },
      size: { value: new Vector2(interactionTarget.width / 2, interactionTarget.height / 2) },
    },
  });

  const renderingMaterial = new RawShaderMaterial({
  ...
  const fboPlane = new Mesh(new PlaneGeometry(2, 2), interactionMaterial);
  fboScene.add(fboPlane);

  const sphere = new Mesh(new SphereGeometry(1, 100, 100, 0, Math.PI), renderingMaterial);
  scene.add(sphere);
  ...
  const onMouseMove = (event) => mousePosition.set(event.clientX, event.clientY);
  const onMouseDown = () => {
    interactionMaterial.uniforms.mouseDown.value = true;
  };
  const onMouseUp = () => {
    interactionMaterial.uniforms.mouseDown.value = false;
  };
  ...
  const render = () => {
    ...
    normalizedMousePosition.set(
      mapRange((mousePosition.x - bounds.left) / bounds.width, 0, 1, -1, 1),
      mapRange((mousePosition.y - bounds.top) / bounds.height, 0, 1, 1, -1),
    );

    if (normalizedMousePosition.x !== -1 && normalizedMousePosition.y !== -1) {
      raycaster.setFromCamera(normalizedMousePosition, camera);
      const intersection = raycaster.intersectObject(sphere)[0];
      if (intersection?.uv) interactionMaterial.uniforms.center.value.copy(intersection.uv);
      else interactionMaterial.uniforms.center.value.set(-1, -1);
    }

    interactionMaterial.uniforms.time.value = clock.getElapsedTime();
    angle += 0.01 * angleDirection;
    if (angle > Math.PI - 0.5 || angle < 0.5) angleDirection *= -1;
    renderingMaterial.uniforms.lightDirection.value.set(Math.cos(angle), Math.sin(angle), 1);
    renderingMaterial.uniforms.angle.value = angle;

    fboPlane.material = interactionMaterial;
    renderer.setRenderTarget(interactionTarget);
    renderer.render(fboScene, camera);
    fboPlane.material = simulationMaterial;
  ...
  const pulseInterval = window.setInterval(() => {
    interactionMaterial.uniforms.center2.value.set(randomBetween(0.5, 1), randomBetween(0, 1));
    pulseTimeout = window.setTimeout(() => interactionMaterial.uniforms.center2.value.set(-1, -1), 10);
  }, 200);
